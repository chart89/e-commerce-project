...
const Order = () => {
  ...
  const products = CartData.map(
    (cart) =>
      cart.id + ',' + cart.sum + ',' + cart.amount + ',' + cart.comments,
  ).join('/');
  //const products = [];
    ...
    <Container>
      {status === 'success' && (
        <Alert className="w-25 my-5 mx-auto" variant="success">
          ...
          <p>Your order has been successfully sending!</p>
        </Alert>
      )}
      <Form onSubmit={validate(handleSubmit)}>
        <Row>
          {CartData &&
            CartData.map((cart) => <OrderList key={cart.id} {...cart} />)}
        </Row>
        <Row className="mb-3">
          <Col></Col>
          <Col xs={4}>
        ...
        <Col className="mb-5 mt-0 w-50 text-end ">
